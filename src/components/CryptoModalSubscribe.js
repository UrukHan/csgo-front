...
import useSubscribeMethods from './SubscribeMethods';
import {AccessContext} from "../AccessContext";
import config from "../utils/config.json";
import {useTranslation} from "react-i18next";

const CryptoModalSubscribe = ({ onClose, onCloseParent }) => {
    ...
    const { solanaSubscribe, evmSubscribe } = useSubscribeMethods(setPaymentStatus, paymentCrypto, transactionsUpdated, onClose, onCloseParent);
    ...
    const handleSubscribeCrypto = async (event) => {
        ...
        if (!isTokenValid) {
            setPaymentStatus('The token is not valid. Please log in again.');
            return;
        }

        if (isSubscribed) {
            setPaymentStatus('You are already subscribed');
            return;
        }
...
export default CryptoModalSubscribe;
